...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if (insect.is_ant):
            ...
            if ((self.ant.name == "Queen") and (self.ant.allQueens[0] == insect)):
                pass
...
class Insect(object):
    ...
    def __init__(self, armor, place=None):
        ...
        #
        if self.watersafe == True:
            Insect.allBees.append(self)
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem 8
        """
        if (self.place.ant != self.place.exit) or ((self.place.ant == True) and (ant.blocks_path == False)):
            return False
        else:
            return True
        """
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        if (self.place.entrance == hive):
            return None
        elif (self.place == self.place.bees):
        ...
        if (self.place == hive):
            return None
        elif (self.place.bees != None):
            return random_or_none(self.place.bees)
        else:
            self.place = self.place.entrance
            ThrowerAnt.nearest_bee(hive)
        """
        #if (self.place == hive) or (moved_steps)
        ...
        while current_place != hive:
            if ((len(current_place.bees) > 0) and (steps_taken >= min_range) and (steps_taken <= max_range)):
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        ...
        #if (self.armor <= 0) and (self.place.bees == self.place):
        newBeeLst = list(self.place.bees)
        for bee in newBeeLst:
...
# END Problem 13
    ...
    def __init__(self):
        ...
        QueenAnt.allQueens.append(self)
        # END Problem 13
    ...
    def action(self, colony):
        ...
        #how to throw leaf?
        """
        if self != QueenAnt.allQueens[0]:
            self.reduce_armor(self.armor)
            self.place.remove_insect(self)
        else:
            ScubaThrower.action(self, colony)
            current_place_2 = self.place
            while current_place_2.exit != None:
                #if current_place_2.exit != colony.hive:
                #    print("oops")
                if ((current_place_2.ant) and (current_place_2.ant != self)):
                    #for ant in current_place_2.ant:
                    if current_place_2.ant.damage_doubled == False:
                        current_place_2.ant.damage *= 2
                        if ((current_place_2.ant.container == True) and (current_place_2.ant.ant != None) and (current_place_2.ant.ant.damage_doubled == False)):
                            current_place_2.ant.ant.damage *= 2
                current_place_2 = current_place_2.exit
                #print(current_place_2.exit)
    """
    ...
    def action(self, colony):
        ...
        if self != QueenAnt.allQueens[0]:
            ...
            current_place_2 = self.place.exit
            
            while current_place_2.exit:
      
                if ((current_place_2.ant not in QueenAnt.ants_dd) and (current_place_2.ant)):
                    if current_place_2.ant != self:
                        current_place_2.ant.damage *= 2
                        QueenAnt.ants_dd.append(current_place_2.ant)
                        
                if ((current_place_2.ant) and (current_place_2.ant.container == True)):
                    if((current_place_2.ant.ant) and (current_place_2.ant.ant not in QueenAnt.ants_dd)):
                        if current_place_2.ant.ant not in QueenAnt.ants_dd:
                            current_place_2.ant.ant.damage *= 2
                            QueenAnt.ants_dd.append(current_place_2.ant.ant)
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 13
        self.armor -= amount
        if QueenAnt.allQueens[0].armor <= 0:
            bees_win()
        # END Problem 13
